...
from zyte_smartproxy_selenium import webdriver # pip install zyte-smartproxy-selenium
...
try:  # Encerramos todo en un try catch para que si no aparece el discilamer, no se caiga el codigo
    ...
    disclaimer = driver.find_element(By.XPATH, '//p[@class="fc-button-label" and text()="Consentir"]')
...
while PAGINACION_MAX > PAGINACION_ACTUAL:
    ...
    links_productos = driver.find_elements(By.XPATH,
                                           '//h2[@class="poly-box poly-component__title"]/a[@href]')
    links_de_la_pagina = []
    ...
    for link in links_de_la_pagina:
        ...
        try:
            ...
            precio_meta = driver.find_element(By.XPATH,
                                              '//div[@class="ui-pdp-price__main-container"]//meta[@itemprop="price"]')

            precio = precio_meta.get_attribute("content")
            print(titulo)
