...
boton_disclaimer = driver.find_element(By.XPATH, '//span[@class="VfPpkd-vQzf8d" and text()="Aceptar todo"]')
...
for url in urls_videos:
    ...
    sleep(3)
    comments = driver.find_element(By.XPATH,'//h2[@id="count"]//span[1]').text
    comentarios_totales = int(comments)*0.9
    print('comentarios : ', comentarios_totales)
    ...
    n_scrolls = 0
    n_scrolls_maximo = 10
    while comentarios_cargados < comentarios_totales and n_scrolls < n_scrolls_maximo:
        driver.execute_script(obtener_script_scrolling(n_scrolls))
        n_scrolls += 1
        sleep(2)
        comentarios_cargados = len(driver.find_elements(By.XPATH,
                                                        '//yt-attributed-string[@id="content-text"]'))
        print('Comentarios cargados', comentarios_cargados)

    comentarios = driver.find_elements(By.XPATH,
                                       '//yt-attributed-string[@id="content-text"]')
